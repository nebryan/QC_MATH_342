...
```{r}
mod_rf = YARF(data.frame(X_train), y_train) # just use defaults (could use m_try)
...
```{r}
mod_rf = YARF(data.frame(Ximp_naive_train), y_train) # just use defaults (could use m_try)
...
Ximp_MF_train = Xyimp_miss[train_idx, 1:col(X)]
Ximp_MF_test = Xyimp_miss[test_idx, 1:col(X)]
```

Fit a random forest model of `y_train ~ Ximp_MF_train`, report oos s_e (not oob) on `Ximp_MF_test`.

```{r}
mod_rf = YARF(data.frame(Ximp_MF_train), y_train) # just use defaults (could use m_try)
...
for(m in 1:length(num_trees_values)){
  mod_bag = YARF(X_train, y_train, num_trees = num_trees_values[m])
  oob_se_rf_mod_by_num_trees[m] = mod_bag$rmse_oob
}
...
There was a percent gain in predictive performance of the RF model over the bagged trees model because the RF model decreases oos error by minimizing the variance term of the MSE. 
...
wide_df = tibble(
  ...
  oob_s_e_bagged = oob_se_bagged_trees_mod_by_num_trees,
  oob_se_rf = oob_se_rf_mod_by_num_trees
)

long_df = melt(wide_df, measure.vars = c("oob_s_e_bagged", "oob_se_rf"))
...
mtry_values = seq(from = 1, to = ncol(diamonds_train), by = 1)

for (m in 1 : ncol(diamonds_train)){
  mod_rf = YARF(X_train, y_train, num_trees = 500, mtry = mtry_values[m]-1)
...
```{r}
ggplot(data.frame(mtry = mtry_values, oob_se = oob_se_by_mtry)) +
  aes(x = mtry, y = oob_se) + 
  geom_point() + geom_line() + scale_x_log10()
```
...
# I dropped the native_country factor variable because it had too many levels (41) 
# I kept on getting an error of "incorrect cutoff specified" when running MissForest

# adult_train = adult %>% sample_n(2000)
adult_train = missForest(data.frame(adult_train))$ximp
X_train = adult_train %>%
  select(-income)
...
for(m in 1:length(num_trees_values)){
  mod_bag = YARF(X_train, y_train, num_trees = num_trees_values[m])
  oob_se_rf_mod_by_num_trees[m] = mod_bag$misclassification_error
}
...
Yes, of course, `mtry` is an important hyperparameter to optimize when using the Random Forest algorithm. 
