...
```{r}
# fitting the 17 models on the diamonds_train set
mods = lapply(model_formulas, lm, diamonds_train) 

# vector containing the number of df from each of the 17 models
df = lapply(mods, function(mod){mod$rank}) 

# finding the oosRMSE on the select set for each model
yhat_selects = lapply(mods, function(mod){predict(mod, diamonds_select)})
y_select = diamonds_select$price #the true prices

# vector of RMSEs on the select set for each model
oosRMSE = lapply(yhat_selects, function(yhat_select){sqrt((1/(n - 10))*(sum((y_select - yhat_select)^2)))})
```
...
X = cbind(df,oosRMSE)
ggplot(X) + 
  aes(x=as.numeric(df), y = as.numeric(oosRMSE)) +
  geom_line() +
  xlim(0,300) + 
  ylim(0,10000)
```
...
```{r}
# select best model by oosRMSE
which.min(oosRMSE)

# oosRMSE for the best model by oosRMSE
mod_for_test = lm(mods[[which.min(oosRMSE)]], rbind(diamonds_train, diamonds_select))
yhat_test = predict(mod_for_test, diamonds_test)
y_test = diamonds_test$price  #true test prices

oosRMSE_test = sqrt((1/(n - 10))*(sum((y_test - yhat_test)^2)))
oosRMSE_test
```

Did we overfit the select set? Discuss why or why not.

No, we did not overfit on the select set since the RMSE of the "best model" we chose on the test set is still lower than the RMSE on the select set. So this model does do a decent job at generalizing based upon its lower generalization error on the test set. 

Create the final model object `g_final`.

```{r}
g_final = lm(mods[[which.min(oosRMSE)]], diamonds_cleaned)
```
...
X[,1] = t(c(rep(0, n))) # had to make the first column into 0's
dim(X)
```
...
select_oosRMSE = array(NA, M)
for (m in 1 : M){
  #train on train set
  ridge_mod = glmnet(X_train, y_train, alpha  = 0, lambda = lambda_grid[m])
  #predict on select set
  y_hat_select = predict(ridge_mod, X_select)
  #measure error from the select set
  select_oosRMSE[m] = sqrt(mean((y_select - y_hat_select)^2))
}
```
...
```{r}
ggplot() + aes(x = lambda_grid, y = select_oosRMSE) +
  geom_line()
```
...
```{r}
# model with the lowest oosRMSE on the select set based upon lambda
best_lambda = which.min(select_oosRMSE) # when lambda = 8

best_ridge_mod = glmnet(X_train, y_train, alpha  = 0, lambda = best_lambda)

y_hat_test = predict(best_ridge_mod, X_test)
...
```{r}
g_final = glmnet(X, y, alpha = 0, lambda = best_lambda)
```
...
```{r}
storms %<>% drop_na(tropicalstorm_force_diameter, hurricane_force_diameter) 
...
```{r}
# mutate category from numeric into a factor data type
storms %<>% mutate(category = factor(category, ordered= TRUE))
storms #DONE
```
...
```{r}
storms_category4_and_wind100_or_above = storms %>% 
  filter(category == c(4,5)) 
storms_category4_and_wind100_or_above #DONE
```
...
```{r}
storms %<>%
  mutate(average_diameter = (tropicalstorm_force_diameter + hurricane_force_diameter)/2)
head(storms) #DONE
```
...
storms %>%
  ...
  select(name, max_wind, year, month, day, everything()) %>%
  arrange(desc(max_wind), year, month, day)
...
storms %>%
  ...
  distinct %>%
  select(year, name) #ALREADY DONE FOR ME
```
...
storms %>%
  ...
  distinct %>%
  arrange(name)
 # Finished but did not max category since I had made it a ordered factor variable
```
...
ggplot(tally_storms) + aes(x = year, y = num_storms) + geom_line() #DONE
```
...
library(lubridate)
time_storms = storms %>%
  mutate(timestamp = ymd_h(paste(year, month, day, hour, sep="."), tz = "US/Eastern")) %>%
...
```{r}
week_and_day_storms = storms %>%
  mutate(day_of_week = factor(weekdays(ymd_h(paste(year, month, day, hour, sep="."), tz = "US/Eastern")))) %>%
  mutate(week_of_year = week(ymd_h(paste(year, month, day, hour, sep="."), tz = "US/Eastern"))) %>%
  select(name, year, day_of_week, week_of_year, month, day)
  
week_and_day_storms #DONE
```

For each storm, summarize the day in which is started in the following format "Friday, June 27, 1975".

```{r}
summary_first_day_storm = storms %>%
  group_by(name) %>%
  slice(1) %>%
  mutate(month = months(ymd_h(paste(year, month, day, hour, sep="."), tz = "US/Eastern"))) %>%
  mutate(day_of_week = factor(weekdays(ymd_h(paste(year, month, day, hour, sep="."), tz = "US/Eastern")))) %>%
  select(name, year, day_of_week, month, day) %>%
  summarize(first_day_of_storm = sprintf("%s, %s %s, %s",day_of_week, month, day, year))
  
summary_first_day_storm #DONE
```
...
```{r}
serious_storms_lat_long = serious_storms %>%
  mutate(lat_long = paste(lat,long,sep="/")) %>%
  select(name, lat_long, everything())

serious_storms_lat_long #DONE
```
